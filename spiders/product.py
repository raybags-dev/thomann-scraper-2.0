...
class ProductProcessorApp:
    ...
    async def download_and_process_page(self, page, url):
        try:
            ...
            await page.goto(url, timeout=randomize_timeout(50000, 70000))
    ...
    async def process_product_endpoints(self, endpoints, concurrency=3):
        async with async_playwright() as p:
            ...
            context = await browser.new_context(extra_http_headers=products_headers_obj.get_product_headers())
    ...
    def save_product_data(self, product_data):
        ...
        emulator(message=f"Processing data objects: {len(product_data)}...", is_in_progress=True)
        ...
        # Clean the data
        product_title = clean_data(product_data.get('product_title', ''))
        review_count = clean_data(product_data.get('review_count', ''))
        product_text_title = clean_data(product_data.get('description_title', ''))
        main_description_text = clean_data(product_data.get('description_text', ''))
        important_note_text = clean_data(product_data.get('product_note', ''))
        product_url = clean_data(product_data.get('product_url', ''))
        product_note = clean_data(product_data.get('badges_list', ''))
        key_features = clean_data(product_data.get('key_features', ''))
        price = clean_data(product_data.get('price', ''))
        disclaimer = clean_data(product_data.get('disclaimer', ''))
        shipping = clean_data(product_data.get('shipping', ''))
        reviews_url = clean_data(product_data.get('reviews_url', ''))
        prediction = clean_data(product_data.get('shipping_prediction', ''))
        ...
        if product_data_json not in self.unique_products:
            ...
            emulator(is_in_progress=False)
            custom_logger(f"Saved product data count: {len(product_data_cleaned)} items", log_type="info")
            # ===================================================
            # ===================================================

    @handle_exceptions
    async def products_main_worker(self, enable_processing=True):

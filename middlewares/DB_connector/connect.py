...
async def handle_db_connection(connect=False):
    ...
    connection_string = mongo_connection_string.replace(credentials, f"{escaped_username}:{escaped_password}")
    ...
    try:
        ...
        if db is not None:
            ...
            custom_logger(f"- keyId: {server_info['$clusterTime']['signature']['keyId']}", log_type="info")

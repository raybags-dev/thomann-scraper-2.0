...
class LoadingEmulator:
    ...
    def update_progress(self):
        ...
        while not self.stopped or percentage <= 95:
            ...
            current_time_str = (
                current_time.strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "+0000"
            )
            print(
                f"\r> {percentage}% - (start_time: {start_time_str}, end_time: {current_time_str}): ",
                end="",
            )
        ...
        if percentage > 95:  # Ensure final completion message shows 100%
            ...
            print(
                f"\r> 100% - (start_time: {start_time_str}, end_time: {current_time_str}): ",
                end="",
            )
    ...
    def print_completion_message(self):
        if self.start_time and self.end_time:
            ...
            print(
                f"\r> 100% - (start_timestamp: {start_time_str}, end_timestamp: {end_time_str}): {Fore.GREEN}> Current process completed.{Fore.RESET}"
            )

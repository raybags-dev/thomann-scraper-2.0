...
def extract_product_data(page_content):
    try:
        ...
        main_container = soup.select_one('.product-main-content.fx-content-product-grid__col')
        ...
        if left_container:
            product_title_element = left_container.select_one('.product-title h1.product-title__title')
            product_title = product_title_element.text.strip() if product_title_element else None
            # ===================================================
            # ===================================================
            review_count_anchor = left_container.select_one('a .fx-rating-stars .product-title__rating-description')
            review_count = review_count_anchor.text.strip() if review_count_anchor else None
            # ===================================================
            # ===================================================
            product_description_element = left_container.select_one('.product-text[itemprop="description"]')
            if product_description_element:
                product_text_title_ele = product_description_element.select_one('.text-original h2.fx-headline')
                product_text_title = product_text_title_ele.text.strip() if product_text_title_ele else None
                # ===================================================
                # ===================================================
                product_description_list = product_description_element.select_one('ul.product-text__list')
                ...
                # ===================================================
                # ===================================================
                important_note = product_description_element.select_one('p.fx-text--plus')
                important_note_text = important_note.text.strip() if important_note else None
                # ===================================================
                # ===================================================
                # Extract badges
                badges_list = []
                ...
                key_features = {}
                key_features_container = product_description_element.select_one('div.keyfeatures')
        ...
        if right_container:
            ...
            if product_price_element:
                price_value_element = product_price_element.select_one('.price-wrapper .price')
                ...
                if url_element:
                    ...
                    review_url = url.replace('.htm', '_reviews.htm?page=1&order=latest&reviewlang%5B%5D=all')

                # ===================================================
                # ===================================================
                disclaimer_container = product_price_element.select_one('.meta')
                if disclaimer_container:
                    disclaimer_meat = disclaimer_container.select_one('.meta__disclaimer')
                    disclaimer = disclaimer_meat.text.strip() if disclaimer_meat else None

                # ===================================================
                # ===================================================
                ship_container = product_price_element.select_one('.price-and-availability__tooltip-wrapper div['
                                                                  'aria-label="tooltip"]')
                if ship_container:
                    ship_ele = ship_container.select_one('span span span.fx-availability')
                    shipping = ship_ele.text.strip() if ship_ele else None

                # ===================================================
                # ===================================================
                ship_prediction = product_price_element.select_one('.shipping-prediction')
                if ship_prediction:
                    pred_text = ''
                    predic_text = ship_prediction.select_one('a').text.strip() if ship_prediction.select_one('a') else ''
                    if predic_text:
                        pred_text += predic_text

                    date_elements = ship_prediction.select_one('strong').find_all('span') if ship_prediction.select_one('strong') else []
                    if date_elements:
                        dates = [span.text.strip() for span in date_elements]
                        if len(dates) == 2:
                            pred_text += f", expected between: {dates[0]} and {dates[1]}"
                else:
                    pred_text = None
            ...
            if ranking_container:
                ranking_obj = {}
                ranking_container.select_one('.product-rank-and-visitors a.meta-box--link')

                rank_value = ranking_container.select_one('.meta-box__value').text.strip() if ranking_container else None
                rank_descript = (ranking_container.select_one('.meta-box__texts .meta-box__description')
                                 .text.strip()) if ranking_container else None
                rank_sub_category = (ranking_container.select_one('.meta-box__texts .meta-box__subtext')
                                     .text.strip()) if ranking_container else None

                rank_url = (ranking_container.select_one('.product-rank-and-visitors a.meta-box--link')
                            .get('href')) if ranking_container else None
